...
/**
 * Created by GuFei on 2018/4/22.
 */
public class MemberWeekDView extends View {
    ...
    private Calendar mSelectedDayForLine;

    private Paint mTimeTextPaint;
    private Paint mCurrentTimeTextPaint;
    private float moveLength; //移动长度

    private float mTimeTextWidth;//时间文本宽度
    private float mTimeTextHeight;//时间文本高度

    private Paint mHeaderTextPaint;
    private Paint mHeaderLabelPaint;
    private float mHeaderTextHeight; //头部文本高度
    ...
    private float mWidthPerDay;//每天的宽度
    private Paint mDayBackgroundPaint;
    private Paint mHourSeparatorPaint;
    private float mHeaderMarginBottom; //头部距离底部间隔
    private Paint mTodayBackgroundPaint;
    private Paint mTodayHeaderTextPaint;
    private Paint mEventBackgroundPaint;
    private Paint mHeaderFocusSameDayBackgroundPaint; //圆
    private float mHeaderColumnWidth; //头部列的宽度
    ...
    private float mDistanceY = 0;  //移动距离Y
    private float mDistanceX = 0;//移动距离X
    private Direction mCurrentFlingDirection = Direction.NONE;

    private boolean mEmptyMark = true;    //非事件点击标识
    private MotionEvent mTouchEvent;    //非事件点击标识
    private RectF mTouchEventRectF = null;    //非事件点击标识
    private double mTouchEventDate;    //非事件点击标识
    private double mTouchEventTime;    //非事件点击标识
    ...
    private int mHeaderDayLabelTextSize = 20;
    private int mHeaderDayTextSize = 18;
    ...
    private int mHourSeparatorColor = Color.parseColor("#666E84");
    private int mHeaderWeekedColor = Color.parseColor("#666E84");
    ...
    private int mHeaderColumnBackgroundColor = Color.parseColor("#0E162D");
    //    private int mHeaderColumnBackgroundColor = Color.RED;
    private int mDefaultEventColor;
    private boolean mIsFirstDraw = true; //首次绘制
...
//            oldX = 0;
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            ...
            if (mCurrentScrollDirection == MemberWeekDView.Direction.NONE) {
                mEmptyMark = true;
                if (abs(distanceX) > abs(distanceY)) {
                    mCurrentScrollDirection = MemberWeekDView.Direction.HORIZONTAL;
                    mCurrentFlingDirection = MemberWeekDView.Direction.HORIZONTAL;


                } else {
                    mCurrentFlingDirection = MemberWeekDView.Direction.VERTICAL;
                    mCurrentScrollDirection = MemberWeekDView.Direction.VERTICAL;
                }
            }
...
//            Log.e("原点1:", mScroller.getCurrY() + "-----------" + e1.getY() + "");
            ...
            if (mCurrentScrollDirection != MemberWeekDView.Direction.VERTICAL) {
                if (abs(x) > 300 && mAreDimensionsInvalidF && abs(moveLength) > 80) {
...
//                Log.e("原点:", oldX + "");
            ...
            } else if (mCurrentScrollDirection == MemberWeekDView.Direction.VERTICAL && mCurrentScrollDirection != MemberWeekDView.Direction.HORIZONTAL) {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            if (mCurrentFlingDirection == MemberWeekDView.Direction.HORIZONTAL) {


            } else if (mCurrentFlingDirection == MemberWeekDView.Direction.VERTICAL) {
...
//            ViewCompat.postInvalidateOnAnimation(WeekNoHeaderView.this);
        ...
        public boolean onSingleTapConfirmed(MotionEvent e) {
            ...
            Calendar selectedTomeForLine = getTimeFromLinePoint(e.getX(), e.getY());
            SimpleDateFormat f = new SimpleDateFormat("yyyy-MM-dd HH:mm", Locale.getDefault());
//
            if (null != selectedTime) mSelectedDay = (Calendar) selectedTime.clone();
            if (null != selectedTomeForLine)
                mSelectedDayForLine = (Calendar) selectedTomeForLine.clone();
...
//            if (mTouchEventRectF != null && mEmptyViewClickListener != null && e.getX() > mHeaderColumnWidth && e.getY() > (mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom)) {
            if (mTouchEventRectF != null && mEmptyViewClickListener != null && e.getX() > mTouchEventRectF.left && e.getX() < mTouchEventRectF.right && e.getY() > mTouchEventRectF.top && e.getY() < mTouchEventRectF.bottom) {
                if (selectedTomeForLine != null) {
                    playSoundEffect(SoundEffectConstants.CLICK);
                    mEmptyViewClickListener.onEmptyViewClicked(mSelectedDayForLine);
                }
                mTouchEventRectF = null;
            }
            if (e.getX() > mWidthPerDay / 2 && e.getY() > mHeaderHeight + 120) {
                mTouchEvent = e;
                Log.e("原点:", f.format(selectedTomeForLine.getTime()));
            }
            invalidate();
...
//            }
    ...
    public MemberWeekDView(Context context) {
        this(context, null);
    }

    public MemberWeekDView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MemberWeekDView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            mHeaderDayLabelTextSize = a.getDimensionPixelSize(Integer.parseInt(String.valueOf(14)), (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 14, context.getResources().getDisplayMetrics()));
            mHeaderDayTextSize = a.getDimensionPixelSize(Integer.parseInt(String.valueOf(18)), (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 18, context.getResources().getDisplayMetrics()));
            ...
            mHeaderWeekedColor = a.getColor(R.styleable.WeekView_headerWeekedColor, mHeaderWeekedColor);
...
//            mHeaderFocusSameDayBackgroundColor = a.getColor(R.styleable.WeekView_headerFocusBackgroundColor, mHeaderFocusSameDayBackgroundColor);
        } finally {
            a.recycle();
        }
    ...
    private void init() {
        ...
        mSelectedDayForLine = (Calendar) mToday.clone();
        ...
        mHeaderTextPaint.setTextSize(mHeaderDayTextSize);
        mHeaderTextPaint.getTextBounds("00:00", 0, "00:00".length(), rect);
        mHeaderTextHeight = rect.height();
        mHeaderTextPaint.setTypeface(Typeface.DEFAULT_BOLD);
        // 标题行的设置。
        mHeaderLabelPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mHeaderLabelPaint.setColor(mHeaderColumnTextColor);
        mHeaderLabelPaint.setTextAlign(Paint.Align.CENTER);
        mHeaderLabelPaint.setTextSize(mHeaderDayLabelTextSize);
        mHeaderLabelPaint.getTextBounds("00:00", 0, "00:00".length(), rect);
        mHeaderTextHeight = rect.height();
        ...
        mHeaderFocusSameDayBackgroundPaint.setColor(Color.GREEN);
...
//        canvas.drawRect(0, mHeaderHeight + mHeaderTextHeight + 80, mTimeTextWidth + mHeaderColumnPadding * 2, mHeaderHeight, mHeaderBackgroundPaint);
...
//        // 不要让视图高于/低于滚动的限制。设置滚动条的最大值和最小值。 调整45部分更高左侧
        if (mCurrentScrollDirection == Direction.VERTICAL) {
            if (mCurrentOrigin.y - mDistanceY > mHeaderHeight + mHeaderTextHeight + 65) {
                mCurrentOrigin.y = mHeaderHeight + mHeaderTextHeight + 65;
//                Log.e("----------------->", "这是向下拉动,到最顶部");
            } else if (mCurrentOrigin.y - mDistanceY < -(mHourHeight * 24 + 40 - getHeight())) {
                mCurrentOrigin.y = -(mHourHeight * 24 + 40 - getHeight());
//                Log.e("----------------->", "滑动到最下方继续滑动");
...
////                Log.e("----------------->", "这是最后一行:"+mCurrentOrigin.y);
        ...
        // 绘制标题栏的背景颜色。
        canvas.drawRect(0, mHeaderMarginBottom + mHeaderHeight + 120, mHeaderColumnWidth,
        ...
        for (int i = 0; i < 24; i++) {
            ...
            if (top < getHeight() && top > mHeaderMarginBottom + mHeaderHeight + 90) {
                    //调整后面偏移量 改变左侧时间轴上移显示的高度

                canvas.drawText(time, mTimeTextWidth + mHeaderColumnPadding, top + mTimeTextHeight + 20, mTimeTextPaint);
                if (!mEmptyMark && !mIsFirstDraw) {
                    Paint p = new Paint();
                    p.setColor(Color.GREEN);// 设置色
                    p.setStrokeWidth(4);
                    p.setStyle(Paint.Style.STROKE);//设置填满

//                    Paint TestP = new Paint();
//                    TestP.setColor(Color.RED);// 设置色
//                    TestP.setStrokeWidth(4);
//                    TestP.setStyle(Paint.Style.FILL);//设置填满

                    Paint TextP = new Paint();
                    TextP.setTextSize(60);
                    TextP.setFakeBoldText(true);
                    TextP.setColor(Color.GREEN);// 设置色

                    mTouchEventTime = Math.floor((mTouchEvent.getY() - (mCurrentOrigin.y - mDistanceY)) / mHourHeight);
                    mTouchEventDate = Math.floor((mTouchEvent.getX() - (mCurrentOrigin.x - mDistanceX)) / (mWidthPerDay + mColumnGap)) + 1;


                    if (mTouchEventTime == i) {

                        SimpleDateFormat format = new SimpleDateFormat("yyyy MM dd", Locale.getDefault());
                        int mTmpOffset = mSelectedDayForLine.get(Calendar.DAY_OF_WEEK);

                        float left = (float) (mTmpOffset * (mWidthPerDay + (mTmpOffset == 1 ? 0 : mColumnGap)) - (mTmpOffset == 1 ? 4 : 8));
//                        float left = (float) (abs((mTouchEventDate) % 7) * (mWidthPerDay +   mColumnGap) - (mTouchEventDate == 1 ? 4 : 8));

                        mTouchEventRectF = new RectF(left, top + 34, (float) (left + mWidthPerDay), (float) top + 34 + getHourHeight());
                        canvas.drawRect(mTouchEventRectF, p);// 长方形
                        canvas.drawText("+", mTouchEventRectF.left + 20, mTouchEventRectF.top + mTimeTextHeight + 40, TextP);
                    }
...
//                mHeaderColumnWidth;
        ...
        int leftDaysWithGaps = (int) -(Math.ceil((mCurrentOrigin.x - mWidthPerDay / 2) / (mWidthPerDay + mColumnGap)));
        float startFromPixel = mCurrentOrigin.x + (mWidthPerDay + mColumnGap) * leftDaysWithGaps +
                mHeaderColumnWidth + ((leftDaysWithGaps + 1) % 7) * (mWidthPerDay / 2);
...
//                canvas.drawLine(startPixel, top, startPixel + mWidthPerDay, top, mCurrentTimeTextPaint);
            ...
            for (int hourNumber = 0; hourNumber < 24; hourNumber++) {
                ...
                float top = mCurrentOrigin.y + mHourHeight * hourNumber + mTimeTextHeight / 2 + mHeaderMarginBottom + mHeaderHeight + 60;
                if (top > 0 + mTimeTextHeight / 2 + mHeaderMarginBottom - mHourSeparatorHeight &&
        ...
        canvas.drawRect(0, 0, getWidth(), mHeaderHeight + mHeaderTextHeight + 120, mHeaderBackgroundPaint);
        ...
        for (int dayNumber = leftDaysWithGaps; dayNumber <= leftDaysWithGaps + mNumberOfVisibleDays; dayNumber++) {
            ...
            boolean selectedDay = isSameDay(mSelectedDayForLine, day);
            // 绘制日期
            String dayLabel = getDateTimeInterpreter().interpretDate(day);
            if (dayLabel == null)
                throw new IllegalStateException("A DateTimeInterpreter must not return null date");

            if (dayNumber == leftDaysWithGaps || dayNumber == leftDaysWithGaps + 6) {
                Log.e("^^^^^^^", dayNumber + "------" + leftDaysWithGaps + mNumberOfVisibleDays);
                mHeaderLabelPaint.setColor(mHeaderWeekedColor);
                mHeaderTextPaint.setColor(mHeaderWeekedColor);
            } else {
            ...
            if (selectedDay) {
                canvas.drawCircle(startPixel + mWidthPerDay / 2,
                        mHeaderTextHeight + mHeaderRowPadding + 105,
                        mHeaderHeight / 2 + 10, mHeaderFocusSameDayBackgroundPaint);
                mHeaderTextPaint.setColor(Color.BLACK);
            }

            canvas.drawText(StringData(day), startPixel + mWidthPerDay / 2, mHeaderTextHeight + 20, mHeaderLabelPaint);
            canvas.drawText(dayLabel, startPixel + mWidthPerDay / 2, mHeaderTextHeight + mHeaderRowPadding + 120, mHeaderTextPaint);
            startPixel += mWidthPerDay + mColumnGap;
        }

        float y = mHeaderTextHeight + 40;
        Paint tmpP = new Paint();
        tmpP.setColor(Color.WHITE);
        canvas.drawLine(0, y, getWidth(), y, tmpP);// 画线
     ...
     * @return The time and date at the clicked position.
     */
    private Calendar getTimeFromLinePoint(float x, float y) {
...
//                    if (hour > 12) hour -= 12;
     ...
     * </p>
     *
     * @param length Supported values are {@link MemberWeekDView#LENGTH_SHORT} and
     *               {@link MemberWeekDView#LENGTH_LONG}.
     */
    @Deprecated
...
//                int nearestOrigin = (int) (mCurrentOrigin.x -  leftDays * (mWidthPerDay + mColumnGap));
                ...
                ViewCompat.postInvalidateOnAnimation(MemberWeekDView.this);
            }
...
//            if (Math.abs(mScroller.getFinalX() - mScroller.getCurrX()) < mWidthPerDay + mColumnGap && Math.abs(mScroller.getFinalX() - mScroller.getStartX()) != 0) {
            if (abs(mScroller.getFinalX() - mScroller.getStartX()) != 0) {
                mScroller.forceFinished(true);
                float leftDays = Math.round(mCurrentOrigin.x / (mWidthPerDay + mColumnGap));
