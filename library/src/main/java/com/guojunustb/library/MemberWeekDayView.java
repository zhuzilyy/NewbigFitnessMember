...
/**
 * Created by GuFei on 2018/4/21.
 */
public class MemberWeekDayView extends View {
...
//
//            if (distanceX>0){
////                mDistanceX = (mWidthPerDay + mColumnGap);
//                mDistanceX = 7;
//            }else{
//                mDistanceX = -7;
//            }
        ...
        public boolean onSingleTapConfirmed(MotionEvent e) {
            ...
            if (mEventRects != null && mEventClickListener != null) {
                ...
                if (mTouchEventRectF != null && mEmptyViewClickListener != null) {
                    if (mTouchEventRectF != null && e.getX() > mTouchEventRectF.left && e.getX() < mTouchEventRectF.right && e.getY() > mTouchEventRectF.top && e.getY() < mTouchEventRectF.bottom) {
                        playSoundEffect(SoundEffectConstants.CLICK);
                        Calendar selectedTime = getTimeFromPoint(e.getX(), e.getY());
                            if (selectedTime != null && mTouchEventTime!=25) {
                                playSoundEffect(SoundEffectConstants.CLICK);
                                mEmptyViewClickListener.onEmptyViewClicked(selectedTime,(int) mTouchEventTime);
                            }
...
//            }
    ...
    public MemberWeekDayView(Context context) {
        this(context, null);
    }

    public MemberWeekDayView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MemberWeekDayView(Context context, AttributeSet attrs, int defStyleAttr) {
            ...
            if (top < getHeight()) {
                ...
                if (!mEmptyMark && !mIsFirstDraw) {
                    ...
                    if (mTouchEventTime == i) {
                        mTouchEventRectF =new RectF(mHeaderColumnWidth, top + 12, getWidth(),(float) top + 12 + getHourHeight());
                        canvas.drawRect(mTouchEventRectF, p);// 长方形
                        canvas.drawText("+", mHeaderColumnWidth+30, top + mTimeTextHeight+60, TextP);
                    }


//                    double mEventY = (mTouchEvent.getY() - mDistanceY);
//                    double top = Math.floor(mEventY / mHourHeight) * mHourHeight + mHeaderMarginBottom;


                    Log.e("~~~~~~~~~~~~~~~~~", "" + top);
                    Log.e("~~~~~~~~~~~~~~~~~", "" + Math.floor((mTouchEvent.getY() - mCurrentOrigin.y - mHeaderMarginBottom) / mHourHeight));
...
//        }
    ...
    private class EventRect {
        ...
        /**
         * 创建事件rect的新实例。EventRect实际上是绘制的矩形。
         * 在给定事件的日历上。单个的矩形可能不止一个。
         * 事件(扩展超过一天的事件)。在这种情况下，有两个例子。
         * EventRect将用于单个事件。给定的事件将被存储在。
         * “originalEvent”。但是对应于矩形实例的事件。
         * 储存在“事件”中。
         *
...
//                    if (hour > 12) hour -= 12;
     ...
     * </p>
     *
     * @param length Supported values are {@link MemberWeekDayView#LENGTH_SHORT} and
     *               {@link MemberWeekDayView#LENGTH_LONG}.
     */
    @Deprecated
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_UP) {
            ...
            if (mCurrentScrollDirection == Direction.HORIZONTAL) {
                ...
                ViewCompat.postInvalidateOnAnimation(MemberWeekDayView.this);
            }
