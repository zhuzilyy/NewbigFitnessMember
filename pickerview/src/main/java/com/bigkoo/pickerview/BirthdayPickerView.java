...
import com.bigkoo.pickerview.view.BirthdayWheelTime;
 ...
 */
public class BirthdayPickerView extends BasePickerView implements View.OnClickListener {
    ...
    BirthdayWheelTime wheelTime; //自定义控件
    private Button btnSubmit, btnCancel; //确定、取消按钮
    private TextView tvTitle;//标题
    public OnTimeSelectListener timeSelectListener;//回调接口
    private int gravity = Gravity.CENTER;//内容显示位置 默认居中
    private BirthdayPickerView.Type type;// 显示类型
    ...
    //构造方法
    public BirthdayPickerView(Builder builder) {
...
//        this.label_seconds = builder.label_seconds;
    ...
    public static class Builder {
        ...
        private BirthdayPickerView.Type type = Type.ALL;//显示类型 默认全部显示
        private int gravity = Gravity.CENTER;//内容显示位置 默认居中
        ...
        private boolean isLeft = false;//是否左侧
        private boolean isRight = false;//是否显示右侧
        ...
        //Option
        public Builder setType(BirthdayPickerView.Type type) {
            this.type = type;
            return this;
        }
    ...
    public Builder isRight(boolean isRight) {
        ...
        public BirthdayPickerView build() {
            return new BirthdayPickerView(this);
        }
    }
    ...
    private void initView(Context context) {
        ...
        init();
        initEvents();
//        wheelTime.setOnClickListener(this);
        if (customListener == null) {
            ...
            btnCancel.setText(TextUtils.isEmpty(Str_Cancel) ? "生日" : Str_Cancel);
        ...
        // 时间转轮 自定义控件
        LinearLayout birthdayPickerView = (LinearLayout) findViewById(R.id.timepicker);

//     timePickerView.setBackgroundColor(Color_Background_Wheel == 0 ? bgColor_default : Color_Background_Wheel);
        birthdayPickerView.setBackgroundColor(Color.parseColor("#FFFFFF"));

        wheelTime = new BirthdayWheelTime(birthdayPickerView, type, gravity, Size_Content);
        ...
        wheelTime.setBGColorCenter(Color.parseColor("#F0F0F0"));
   ...
   public void setDate(Calendar date) {
    ...
    public void returnData() {
        if (timeSelectListener != null) {
            try {
                Date date = WheelTime.BirthdaydateFormat.parse(wheelTime.getTime());
